...
const Category = () => {
    ...
    const handleDelete = async (values) => {
        try {
            const response = await deleteApiCall(`${ROUTES_URL.CATEGORY_URL}/${values._id}`);
            if (response.data) {
                showNotification({
                    type: "success",
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (mode === "add") {
                ...
                const response = await postApiCall(ROUTES_URL.CATEGORY_URL, payload, true);
                ...
                const response = await putApiCall(`${ROUTES_URL.CATEGORY_URL}/${categoryId}`, payload, true);
                console.log('response',response);
                if (response) {
                    showNotification({
                        type: "success",
    ...
    const getCategoryData = async () => {
        try {
            setLoading(true);
            const response = await getApiCall(ROUTES_URL.CATEGORY_URL, true);
            console.log('response', response);
            if (response) {
                const totalData = response?.data.data;
                const newData = totalData.map((item, i) => ({ ...item, themeTitle: item.themeId.themeTitle, key: i, srno: i + 1 }));
    ...
    return (
        <>
            ...
            >
                ...
                <CategoryForm mode={mode} themeList={themeList} onFinish={onFinish} form={form} loading={loading} onClose={onClose} />
            </Drawer>
        </>
    );
};
