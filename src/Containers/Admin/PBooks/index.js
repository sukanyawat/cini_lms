...
const Book = () => {
    ...
    const [filerRackList, setFilerRackList] = useState([]);
    ...
    const handleUpdate = (record) => {
        ...
        if (record._id) {
            ...
            form.resetFields();
            form.setFieldsValue({...record, categoryId: record.categoryId._id, themeId: record.themeId._id, subcategoryId: record.subcategoryId?._id, rackId: record.rackId?._id,})
        }
        setOpen(true);
    };
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            if (mode === "add") {
                ...
                formData.append("bookType", "pbook");
    ...
    const getBookData = async () => {
        try {
            ...
            setLoading(true);
            const response = await getApiCall(`${ROUTES_URL.BOOK_URL}?type=pbook&${queryString}`, true);
            if (response) {
                const totalData = response?.data.data.books;
                const newData = totalData.map((item, i) => ({ ...item, key: i, srno: i + 1, categoryName: item.categoryId.categoryName, themeTitle: item.themeId.themeTitle, rackName: item.rackId.rackName }));
                setBookData(newData);
            }
        } catch (err) {
            setLoading(false);
    ...
    const getRackList = async () => {
        try {
            ...
            if (response && response.data.ack == 'ok') {
                ...
                const rackList = response?.data.data.map(val => ({
                    value: val._id,
                    label: val.rackName
                }));
                setFilerRackList([...rackList]);
            }
        } catch (err) {
            console.log('Error => ', err);
        }
        ...
        <>
            <div className={styles.container}>
                <div className={styles.flex_container}>
                    <p className={styles.title}>Physical Book List</p>
                    <div>
            ...
            <Drawer
                ...
                title={mode === 'add' ? "Add Physical Book" : "Update Physical Book"}
                width={400}
                onClose={onClose}
                visible={open}
                styles={{
